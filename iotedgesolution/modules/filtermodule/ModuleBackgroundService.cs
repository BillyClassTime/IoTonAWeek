    ...
    async Task<MessageResponse> FilterMessages(Message message, object userContext)
    {
        ...
        try
        {
            ...
            if (messageBody != null && messageBody.machine!.temperature > temperatureThreshold)
            {
                ...
                //var command = new ControlCommand { Command = ControlCommandEnum.Reset};
                //var commandJson = JsonConvert.SerializeObject(command);
                var commandJson = @"{""command"":""Reset""}";
                var messageControl = new Message(Encoding.UTF8.GetBytes(commandJson));
                await moduleClient.SendEventAsync("control", messageControl);
            }
...
class MessageBody
{
    ...
    public Ambient? mmbient { get; set; }
